        ...
        <>
            <tr className='room'>
                <td >
                    ...
                    {
                        roomInfo?.ThongTinGiuong?.map((giuong, index) => <div key={index} className='room__bed'>{giuong.SoLuong}  {giuong.Giuong.TenLoaiGiuong}</div>)
                    }
                ...
                <td>
                    {
                        new Array(roomInfo?.SoLuongKhach).fill("").map(() => <span>{ICONS.PERSON} </span>)
                    }
                </td>
                <td>
                    ...
                    <Select defaultValue={0} style={{ minWidth: "200px" }}>
                        <Select.Option value={0}>0</Select.Option>
                        {
                            new Array(roomInfo.SoLuongPhong).fill().map((room, index) =>
                                <Select.Option value={JSON.stringify({ option: index + 1, price: roomInfo.Gia * (index + 1) })} >{index + 1}  (VND&nbsp;{convertToMoney(roomInfo.Gia * (index + 1))})</Select.Option>
                            )
                        }
                    </Select>
                </td>
