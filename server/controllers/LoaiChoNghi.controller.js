...
module.exports = {
  ...
  get: async (req, res) => {
    try {
      ...
      const LoaiChoNghi = await LoaiChoNghiModel.findOne({
        _id: MaLoaiChoNghi,
      });
      if (!LoaiChoNghi)
        return res
          .status(400)
          .json({ message: "Loại chổ nghĩ không tồn tại !" });
  ...
  patch: async (req, res) => {
    try {
      const { TenLoaiChoNghi } = req.body;
      const { MaLoaiChoNghi } = req.params;
      let LoaiChoNghi = null;
      if (req.file) {
        LoaiChoNghi = await LoaiChoNghiModel.updateOne(
          { _id: MaLoaiChoNghi },
          { TenLoaiChoNghi, HinhAnh: req.file.path }
        );
      } else {
        LoaiChoNghi = await LoaiChoNghiModel.updateOne(
          { _id: MaLoaiChoNghi },
  ...
  delete: async (req, res) => {
    try {
      const { MaLoaiChoNghi } = req.params;
      const LoaiChoNghi = await LoaiChoNghiModel.deleteOne({
        _id: MaLoaiChoNghi,
      });
      if (LoaiChoNghi.deletedCount === 0)
        return res
          .status(400)
          .json({ message: "Loại chổ nghĩ không tồn tại !" });
